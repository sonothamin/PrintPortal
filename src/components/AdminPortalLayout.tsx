...
export default function AdminPortalLayout({ children }: { children: React.ReactNode }) {
  ...
  const drawerContent = (
    <>
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', justifyContent: (open || isMobile) ? 'space-between' : 'center', minHeight: 64 }}>
        {(open || isMobile) && (
        ...
        {!isMobile && (
          <IconButton onClick={() => setOpen(!open)} size="small" aria-label={open ? "Collapse sidebar" : "Expand sidebar"}>
        ...
        {adminMenuItems.map((item) => (
          <ListItem key={item.text} disablePadding sx={{ mb: 0.5 }}>
            <Link href={item.path} passHref style={{ width: '100%' }}>
              ...
              >
                <ListItemIcon sx={{ minWidth: 0, mr: (open || isMobile) ? 2 : 'auto', justifyContent: 'center', color: 'inherit' }}>
                  {item.icon}
                </ListItemIcon>
                {(open || isMobile) && <ListItemText primary={item.text} primaryTypographyProps={{ fontWeight: 700, fontSize: '0.875rem' }} />}
              </ListItemButton>
            </Link>
          </ListItem>
        ))}
      ...
      <Box sx={{ mt: 'auto', p: 2 }}>
        ...
        <Divider sx={{ mb: 1 }} />
        <ListItemButton onClick={handleLogout} sx={{ borderRadius: 2, color: 'error.main' }} aria-label="Sign out">
          <ListItemIcon sx={{ minWidth: 0, mr: (open || isMobile) ? 2 : 'auto', color: 'inherit' }}><LogOut size={20} /></ListItemIcon>
          {(open || isMobile) && <ListItemText primary="Sign Out" primaryTypographyProps={{ fontWeight: 700 }} />}
        </ListItemButton>
      </Box>
    </>
  );
  ...
  return (
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', width: isMobile ? '100%' : `calc(100% - ${open ? drawerWidth : 72}px)` }}>
        <AppBar position="sticky" elevation={0} sx={{ bgcolor: alpha(theme.palette.background.default, 0.8), backdropFilter: 'blur(12px)', borderBottom: '1px solid', borderColor: 'divider', color: 'text.primary', zIndex: 10 }}>
          <Toolbar sx={{ justifyContent: isMobile ? 'space-between' : 'flex-end', px: { xs: 2, sm: 4 }, gap: 2 }}>
            {isMobile && (
              <IconButton onClick={() => setMobileOpen(true)} size="small" aria-label="Open menu">
                <MenuIcon size={20} />
              </IconButton>
            )}
            
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexGrow: isMobile ? 0 : 1 }}>
              <Box sx={{ position: 'relative', width: '100%', maxWidth: 400, display: { xs: 'none', md: 'block' } }}>
