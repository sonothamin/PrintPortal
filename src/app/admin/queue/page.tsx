...
import { 
  ...
  Alert, Tooltip, CircularProgress, Divider, Select, FormControl
} from '@mui/material';
import { 
...
function GlobalQueueContent() {
  ...
  const updateJobStatus = async (jobId: string, newStatus: string) => {
    ...
    if (error) {
      setStatusMsg({ text: `Failed to update status: ${error.message}`, type: 'error' });
    } else {
  ...
  const handlePurge = async (funcName: 'clear-temp-files' | 'clear-queue-uploads') => {
    ...
    try {
      ...
      setStatusMsg({ text: `Purge complete. ${data.count || 0} files removed.`, type: 'success' });
    } catch (err: any) {
  ...
  const statusMap: Record<string, { color: "warning" | "info" | "success" | "error", icon: any }> = {
    ...
    <AdminPortalLayout>
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 2 }}>
        ...
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" color="error" startIcon={<Trash2 size={18} />} onClick={() => setQueueModalOpen(true)} sx={{ borderRadius: 2, fontWeight: 700 }}>Clear Storage</Button>
          <Button variant="outlined" color="warning" startIcon={<AlertTriangle size={18} />} onClick={() => setTempModalOpen(true)} sx={{ borderRadius: 2, fontWeight: 700 }}>Clear Temp</Button>
          <Button variant="contained" startIcon={<RefreshCw size={18} />} onClick={fetchJobs} sx={{ borderRadius: 2, fontWeight: 700 }}>Refresh</Button>
        </Box>
      </Box>
              ...
              <TableRow key={job.id} hover>
                <TableCell>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Box sx={{ p: 1, bgcolor: alpha(theme.palette.primary.main, 0.1), borderRadius: 2 }}><FileText size={20} color={theme.palette.primary.main} /></Box>
                    <Box>
                ...
                <TableCell>
                   <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><User size={14} /><Typography variant="body2">{job.profiles?.full_name || 'Guest'}</Typography></Box>
                </TableCell>
                <TableCell>
                    ...
                    <Select
                      ...
                      renderValue={(selected) => (
                        <Chip 
                          label={selected} 
                          size="small" 
                          icon={statusMap[selected]?.icon}
                          color={statusMap[selected]?.color}
                          sx={{ fontWeight: 700, textTransform: 'uppercase', fontSize: '0.6rem', height: 24 }}
                        />
                      )}
                      sx={{ 
                        height: 32, 
                        ...
                        bgcolor: alpha(theme.palette[statusMap[job.status].color].main, 0.08)
                      }}
                    >
                      {Object.keys(statusMap).map((status) => (
                        <MenuItem key={status} value={status} sx={{ gap: 1, fontWeight: 600, fontSize: '0.8rem' }}>
                          {statusMap[status].icon} {status.toUpperCase()}
                        </MenuItem>
                      ))}
                    </Select>
                  </FormControl>
                  ...
                  <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end' }}>
                    {job.status === 'pending' && (
                      <Button 
                        ...
                        sx={{ borderRadius: 1.5, fontWeight: 800, bgcolor: 'text.primary', color: 'background.paper' }}
                      >
                        {releasing === job.id ? <CircularProgress size={16} color="inherit" /> : 'Release'}
                      </Button>
                    )}
                    <Tooltip title="Release QR">
                      <IconButton size="small" onClick={() => { setSelectedJob(job); setQrModalOpen(true); }} sx={{ border: '1px solid', borderColor: 'divider', color: 'primary.main' }}>
                        <QrCode size={16} />
                      </IconButton>
                    </Tooltip>
                    <IconButton size="small" sx={{ border: '1px solid', borderColor: 'divider' }}><Eye size={16} /></IconButton>
                  </Box>
                </TableCell>
              </TableRow>
            ))}
      ...
      <Dialog open={qrModalOpen} onClose={() => setQrModalOpen(false)} PaperProps={{ sx: { borderRadius: 5, p: 1 } }}>
        ...
        <DialogContent sx={{ textAlign: 'center', pt: 0 }}>
          ...
          <Typography variant="body2" color="text.secondary">Scan to verify job release</Typography>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={tempModalOpen} onClose={() => setTempModalOpen(false)} PaperProps={{ sx: { borderRadius: 2 } }}>
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button variant="contained" color="error" onClick={() => handlePurge('clear-temp-files')} disabled={clearingTemp}>Confirm</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog open={queueModalOpen} onClose={() => setQueueModalOpen(false)} PaperProps={{ sx: { borderRadius: 2 } }}>
        ...
        <DialogActions sx={{ p: 2 }}>
          ...
          <Button variant="contained" color="error" onClick={() => handlePurge('clear-queue-uploads')} disabled={clearingQueue}>Confirm</Button>
        </DialogActions>
      </Dialog>
    </AdminPortalLayout>
  );
